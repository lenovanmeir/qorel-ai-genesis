    ...
    <section id="services" className="relative py-28">
      <div className="mx-auto max-w-7xl px-5">
        <div className="flex flex-col items-end justify-between gap-6 md:flex-row md:items-end">
          ...
          <p className="max-w-md text-muted-foreground">{t.services.lead}</p>
        </div>
        ...
        <div className="mt-14 grid gap-px overflow-hidden rounded-2xl bg-border sm:grid-cols-2 lg:grid-cols-4">
          {t.services.items.map((s, i) => {
            const Icon = icons[i];
            return (
              <div key={s.title} className="group relative bg-background p-7 transition-colors hover:bg-card">
