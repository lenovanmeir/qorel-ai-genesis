    ...
    <section id="cta" className="relative py-32">
      <div className="mx-auto max-w-6xl px-5">
        <div className="relative overflow-hidden rounded-3xl glass-strong p-10 text-center shadow-elevated md:p-20 border-gradient">
          ...
          <p className="text-xs font-medium uppercase tracking-[0.25em] text-primary">{t.finalCta.eyebrow}</p>
          <h2 className="mx-auto mt-5 max-w-3xl text-4xl font-semibold tracking-tight md:text-6xl">
            {t.finalCta.title1}<span className="text-gradient-primary">{t.finalCta.title2}</span>
          </h2>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">{t.finalCta.subtitle}</p>
          ...
          <p className="mt-8 text-xs text-muted-foreground/80">{t.finalCta.footnote}</p>
        </div>
      </div>
    </section>
  );
